...
You can use commands in your Terminal (Mac and linux: Terminal, Windows: Windows Terminal or Powershell equivalent) as an alternative to graphical applications. Since the connection will be terminated if your computer sleeps, terminal crashes, network drops out etc, this method is not particualrly robust for large transfers. For larger transfers rather use the queue based methods (either `copyq` or Gadi or `dt-queue` on Artemis) described below.

Using programs like `tmux` or `screen` can help (see below), however note that the login server has finite capacity, and transfer speeds can be seriously impacted during times of high load. 
...
::: {.callout-note}
## How to transfer from RDS to Gadi (and vice-versa) at a Gadi terminal
...
#### Transfers from a terminal on Artemis using rsync - PRIOR TO DECOMISSION ONLY
...
You can also initiate the transfer in a terminal by connecting via ssh to `research-data-int.sydney.edu.au` (on campus or USyd VPN) and using the `rsync` method described below for Artemis logins.
:::
