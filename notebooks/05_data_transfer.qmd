...
find -type f \( -not -name "myData_md5sum.md5" \) -exec md5sum '{}' \; > myData_md5sum.md5
```

This will generate a file called `myData_md5sum.md5` in your folder listing a hash for every file (except for the `myData_md5sum.md5` file).
...
md5sum -c myData_md5sum.md5
```

This command goes through the list of files in `myData_md5sum.md5` and compares the md5 sum of the file on Gadi to the md5 sum that was created for the file on RDS. 
...
```bash
md5sum -c myData_md5sum.md5 > myData_md5sum.md5.check
grep FAILED myData_md5sum.md5.check > myData_md5sum.md5.checkFAILED
```
...
On Gadi, you can create the checksum files with the same command as above. You can run this on the Gadi login node, or run the command in a Gadi [job script](./08_job_script.qmd). 
