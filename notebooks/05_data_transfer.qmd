...
In this section, we will look at Data Transfer. For the remaining challenges, please visit the specific linked content. We will run training sessions on some of these during the lead up to the Artemis decomission date.
...
/g/data is inteded to be used to store longer term files that are reqularly used by your project during its lifetime. Always ensure your data is regularly backed up from here to the Research Data Store.
...
Depending on the size and complexity of the data you are transferring you have multiple options available:
  ...
  + For large transfers (>100GB) you sould use the data transfer queue options on either Gadi (`dataq`) or Artemis (`dtq`). 
...
::: {.callout-warning}
This option is only available prior to the decommission of Artemis on 29 August 2025 - after that date you will have to use either GLOBUS (*preferred*), or copy data when logged into the Gadi terminal (wither at the login shell or using `copyq` scripts).
:::
...
::: {.callout-warning}
While this method can help you run your copy job in the background over a long period of time, it is reccommended to rather use the `copyq` transfer method described below for large file transfers.
:::
...
You can make a copy of these scripts to your /scratch/<nci-project-code> or /home/<nci-user-id> workspace on Gadi and edit to suit your needs.
...
1. Use `from_gadi_to_rds.pbs` script
...
1. Use `from_rds_to_gadi.pbs` script
2. Replace variables in `<brackets>` as described above 
...
lftp is slower but does sync
sftp is faster but doesent.

suggested workflow is to copy the data in bulk from rds using sftp first and the keep it up-to-date with rds by syncing the data as required with lftp.
