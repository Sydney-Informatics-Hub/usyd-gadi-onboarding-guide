...
- [Adjusting PBS directives to suit Gadi requirements and queue structure](./08_job_script)
- [Lack of internet access for Gadi compute nodes](./08_job_scripts)
- Data transfer
...
Your home space (`/home/<username>`) is owned by you and has 10 GiB of available space. This cannot be increased.
...
Your /g/data space (`/g/data/<project>`) is owned by your project and has the available space allocated by the Sydney Scheme Manager. It is equivalent of `/project` on Artemis.
...
Your scratch space (`/scratch/<project_id>`) is owned by your project and has 1 TiB of available space which can be increased upon request to NCI. It is roughly equivalent to `/scratch` on Artemis.

Data are not backed up and files not accessed for 100 days will be quarantined for 14 days and then removed. Scratch should be used for temporary files associated with a job that has large data IO and not for longer term storage. Always ensure any data you need from a job that is left in /scratch is promptly backed up to the Research Data Store.
...
Depending on the size and complexity of the data you are transferring you have multiple options available:
  ...
  + For large transfers (>100GB) you should use the data transfer queue options on either Gadi (`dataq`) or Artemis (`dtq`). 

::: {.callout-warning}
All data transfer to and from Gadi should be made using the "Data Mover Node" at `gadi-dm.nci.org.au` where possible rather than the login nodes. This ensures that data transfer will not consume resources on the otherwise limited login nodes.
:::
...
For smaller files or datasets, for example your code that you are going to run, you can map your RDS project as a network drive and transfer the data to Gadi via an intermediate data transfer client GUI such as `filezilla` or `cyberduck`.
...
You can use commands in your terminal application (Mac and linux: Terminal, Windows: Windows Terminal or Powershell equivalent) as an alternative to graphical applications. However, since the connection will be terminated if your computer sleeps, terminal crashes, network drops out etc, this method is not particularly robust for large transfers. For these rather use the queue based methods (either `copyq` or Gadi or `dt-queue` on Artemis) described below.
...
::: {.callout-warning}
Please note that running these commands on the login nodes are not the recommended way to transfer research data to Gadi! For smaller downloads, this is OK, but for normal purposes the use of Gadi’s `copyq` and data mover nodes are the appropriate tools.
:::
...
To transfer data between RDS and Gadi from the Gadi login shell:
  ...
  ```bash
  ssh <NCI userid>@gadi.nci.org.au
  ```
  ...
  ```bash
  sftp -r <your_unikey>@research-data-ext:/rds/PRJ-<Project Short ID>/<Path to File or Folder> <location on Gadi>
  ```
  ...
  ```bash
  sftp -r <my_unikey>@research-data-ext:/rds/PRJ-Training/MyData /scratch/qc03/MyData
  ```
...
If you want to copy the other way around (ie. from Gadi /scratch to RDS) use
  ...
  ```bash
  sftp <your_unikey>@research-data-ext:/rds/PRJ-<Project Short ID>/<Path to File or Folder> <<< $"put -r <location on Gadi>"
  ```
:::
...
When logged into Artemis you can use the `rsync` or `scp` command to copy data directly to/from Gadi, since Gadi allows the kind to secure connection that these commands require.
...
To transfer data between RDS and Gadi from the Artemis login shell:
  ...
  ```bash
  rsync -rtlPvz /rds/PRJ-<Project Short ID>/<Path to File or Folder in RDS> <nci login>@gadi-dm.nci.org.au:<Destination on Gadi>
  ```
  ...
  ```bash
  rsync -rtlPvz /rds/PRJ-Training/MyData <nci login>@gadi-dm.nci.org.au:/scratch/qc03/MyData
  ```
...
If you want to copy the other way around (ie. from Gadi /scratch to RDS) then simply reverse the order of the above command in step 2, e.g:
  ...
  ```bash
  rsync -rtlPvz <nci login>@gadi-dm.nci.org.au:<Source on Gadi> /rds/PRJ-<Project Short ID>/<Destination on RDS>
  ```
:::

#### Using `screen` to run your job in the background
...
You can run all of the above copy commands in a `screen` session which can be *detached* to the background allowing you to log out of Gadi and switch off your computer while the copy still runs in the background.

To do this, after logging in (step 1. above) enter the command `screen` at the prompt. This will send you into a screen terminal session, then inside the screen session enter the copy command (step 2. above) and the copy will start. Then while the copy command is running, enter `<ctrl> - d` to detach the screen session and return to the login prompt. You can now log out of Gadi while your copy job is running in the background on Gadi. To check its status simply log back into Gadi in a terminal (step 1. above) and enter the command `screen -r`. This will re-attach your running screen session and you can investigate its output to check if its done. When things are finished you can exit the running screen session by typing `exit` inside it.
...
::: {.callout-warning}
Before you use the scripts below with `copyq`, you will need to set up SSH keys first to enable passwordless login. See the section **Set up SSH keys for passwordless data transfer** for detailed step-by-step instructions in how to do this. You only need to set this up once.
:::
...
# <path>        : The location on RDS to put your folder
...
Here is an example showing you how to transfer a folder called `MyData` in the RDS project `MYPROJECT` to
...
# Make a folder called workspace in /scratch
mkdir /scratch/aa00/workspace
```

Copy the required data transfer script template from /scratch/qc03 to your newly made workspace. In this case we ar copying from RDS
...
then save the edited script to your workspace.

```bash
cp /scratch/qc03/data-transfer-scripts/gadi-scripts/from_rds_to_gadi.pbs /scratch/aa00/workspace
```
...
In the `# PBS` variables part of the script: 
 ...
 + Alter `-lstorage=scratch/<project>` as required. If you also need to access g/data, you can change this to `scratch/<project>+/gdata/<project>`. In this example we'll just use `/scratch/aa00`

In the body of the script: 

 + Provide the `remote_user` variable by replacing `<unikey>` with your USyd unikey.

 + Provide the `remote_path` variable by replacing `<rds_project>` and `<local_path>` with your RDS project name and path to the file or directory you want to transfer. In this example we use `remote_path=/rds/PRJ-MYPROJECT/MyData`
...
remote_host=research-data-ext.sydney.edu.au
remote_path=/rds/PRJ-MYPROJECT/MyData
...
When you have finished editing the script save it (using `<ctrl> - x` and answering `Y` at the prompt)
...
To confirm the transfer was successful, you'll need to check your job logs. These are located in the same directory as your script and are named `transfer.o<jobid>`. Check for **Exit status: 0**. If you see this, the transfer was successful. 
...
#### Template copyq scripts for sync-ing data with `lftp`
