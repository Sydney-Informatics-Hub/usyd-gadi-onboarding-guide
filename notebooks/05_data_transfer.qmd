...
Data are not backed up and files not accessed for 100 days will be quarantined for 14 days and then removed (See [here](https://opus.nci.org.au/spaces/Help/pages/241926268/Recover+Files...RecoverFiles...-RecoverQuarantinedFilesonscratch) for instructions for removing files from quarantine). Scratch should be used for temporary files associated with a job that has large data IO and not for longer term storage. Always ensure any data you need from a job that is left in /scratch is promptly backed up to the Research Data Store.
...
Depending on the size and complexity of the data you are transferring you have multiple options available:
  ...
  + For large transfers you should use the data transfer queue options on either Gadi (`dataq`) or Artemis (`dtq`). 
