...
Enter your unikey password, and once connected, navigate to the folder containing the data you are going to transfer to Gadi. 
...
```bash
cd /scratch/<nci-project-id>/DataDir
md5sum -c my_folder_md5sum.md5
```

This command goes through the list of files in `my_folder_md5sum.md5` and compares the md5 sum of the file on Gadi to the md5 sum that was created for the file on RDS. 

For larger files, or very large numbers of files, the checksum process can take some time (eg > 10 minutes for a single 100 GB file) so it is more robust to submit the checksum job to the Gadi job scheduler. 
...
You can also redirect the output of the `md5sum -c` command to a file, which can simplify collecting failed transfers for resubmission:

```bash
md5sum -c my_folder_md5sum.md5 > my_folder_md5sum.md5.check
grep FAILED my_folder_md5sum.md5.check > my_folder_md5sum.md5.checkFAILED
```

Any files with `FAILED` status should be re-copied from RDS to Gadi, and the checksum run again. 

### Using md5sum to validate files: from Gadi to RDS

The high-level process is the same as for RDS to Gadi, but the **source** and **destination** are reversed. 

##### 1. Create a list of checksums for your files at the source (before copying the data)

On Gadi, you can create the checksum files with the same command as above, however you also have the option to run the command in a Gadi [job script](./08_job_script.qmd). 
...
Enter your unikey password, and once connected, navigate to the folder containing the data you have just transferred across to Gadi. 
...
### Alternate methods of checksum command on RDS
