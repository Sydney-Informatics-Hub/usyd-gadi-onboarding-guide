...
Before submitting your parallel job, **always check** that the [task script reads in the input arguments correctly](#checking-the-parallel-job-before-submission) and that the [binding of tasks to resources is correct]. #TODO
...
### Check the binding of resources to tasks
...
PBS_NCPUS=48
ncores_per_task=8
ncores__per_numanode=12

mpirun -np $((PBS_NCPUS/ncores_per_task)) --map-by ppr:$((ncores_per_numanode/ncores_per_task)):NUMA:PE=${ncores_per_task}
mpirun -np (48/8) --map-by ppr:(12/8):NUMA:PE=8
mpirun -np 6 --map-by ppr:1:NUMA:PE=8
```

This equation is asking for 6 tasks to be run concurrently (`-np 6`) with 1 x 8 CPU task per NUMA node (`ppr:1:NUMA`). The NUMA node can run a single 8 CPU task, but there have not been sufficient NUMA nodes requested by the job to run 6 tasks concurrently with the restriction of one task per NUMA. 

To enable this job to run with the desired configuration, the equation to work out the value for `-np` must be changed:

```bash
-np $((PBS_NCPUS/ncores_per_numanode))
-np (48/12)
-np 4
```
...
- PBS job arrays are not permitted on Gadi, these can be adapted to `nci-parallel` jobs
...
- To maximise efficiency, sort tasks by longest running to shortest running if known in advance
